...
setwd("~/R_Coursera_Course/UCI HAR Dataset")
...
x_Train$DataType <- as.factor(x_Train$DataType)
x_Test$DataType <- "TEST"
x_Test$DataType <- as.factor(x_Test$DataType)
...
#         step #4 in the original instrucitons.  In addition, this set of variables are defined as
#         factors to facilitate the tidy data summarization process. Placed at the front of the
#         processing cycle to trim down the data set early for performance advantages.)
...
subject_test$SubjectId <- as.vector(subject_test$SubjectId)
names(subject_Train) <- c("SubjectId")
subject_train$SubjectId <- as.vector(subject_train$SubjectId)
# Step #2: Extract only the measurements on the mean and standard deviation for each measurement.
#         This step reduces the working dataset to just the principlal fields required for the
#         assignment.
focusFeatures <-
  subset(features, (!grepl("meanfreq", tolower(features$V2))
                    & !grepl("angle", tolower(features$V2)))
         &
           (
             grepl("mean", tolower(features$V2)) |
               grepl("std", tolower(features$V2)) |
               grepl("ActivityId", tolower(features$V2)) |
               grepl("SubjectId", tolower(features$V2))
           ))
...
tiddy_mean <-
  ddply(tiddy_Data,.(Activity),numcolwise(mean,na.rm = TRUE))
tiddy_mean$SubjectId <- NULL

# Step 6: Write the fully tiddy_data and tiddy_mean files to the file system
