...
Y_test <- read.table("test/y_test.txt", colClasses = c("factor"), col.names = c("activityId"))
X_train <- read.table("train/X_train.txt")
Y_train <- read.table("train/y_train.txt", colClasses = c("factor"), col.names = c("activityId"))
subject_Test <- read.table("test/subject_test.txt", colClasses = c("factor"), col.names = c("subjectId"))
subject_Train <- read.table("train/subject_train.txt", colClasses = c("factor"), col.names = c("subjectId"))
...
features <- read.table("features.txt", colClasses = c("numeric","factor"), col.names= c("colId","featureLabel"))
activity_labels <- read.table("activity_labels.txt", colClasses = c("numeric","factor"), col.names=c("activityId","activityLabel"))
...
#         assignment.
focusFeatures <- subset(features, (!grepl("meanfreq", tolower(features$featureLabel)) 
                        & !grepl("angle", tolower(features$featureLabel)))
             & 
               (grepl("mean", tolower(features$featureLabel)) | grepl("std", tolower(features$featureLabel)) | 
                  grepl("ActivityId", tolower(features$featureLabel)) | grepl("SubjectId", tolower(features$featureLabel))))
